#!/usr/bin/env python3
"""Refine D01/D02 wide-table features with global correlation, D03, D04, and D05.

This script pulls a sampled dataset from the DP wide table through TMLSQL only
when explicitly executed. It is intended for the post-D01/D02 convergence stage:
...
def make_dataset_parts(df: pd.DataFrame, x: pd.DataFrame, cfg: dict[str, Any]) -> DatasetParts:
    ...
    if not train_mask.any() or not valid_mask.any():
        raise RuntimeError("Both train and valid splits are required for D03/D04/D05.")
    return DatasetParts(
